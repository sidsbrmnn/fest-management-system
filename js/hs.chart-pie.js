    ...
    $.HSCore.components.HSChartPie = {
        ...
        initCircles: function () {
            ...
            if (lastItem.length) {
                lastId = +lastItem
                    .attr('id')
                    .substring(lastItem.attr('id').lastIndexOf('-') + 1);
            }
            ...
            this.collection.each(function (i, el) {
                ...
                var circle = Circles.create({
                    ...
                    text: function (value) {
                        if ($this.data('circles-type') == 'iconic') {
                            ...
                            if (
                                $this.data('circles-additional-text-type') ===
                                'prefix'
                            ) {
                                if ($this.data('circles-secondary-text')) {
                                    return (
                                        ...
                                        ($this.data('circles-is-hide-value')
                                            ? ''
                                            : value) +
                                        '<div style="margin-top: ' +
                                        ($this.data('circles-divider-space') /
                                            2 +
                                            'px' || '0') +
                                        '; margin-bottom: ' +
                                        ($this.data('circles-divider-space') /
                                        ...
                                        $this.data(
                                            'circles-secondary-font-weight'
                                        ) +
                                        '; font-size: ' +
                                        $this.data(
                                            'circles-secondary-font-size'
                                        ) +
                                        'px; color: ' +
                                        $this.data('circles-secondary-color') +
                                        ';">' +
                                        $this.data('circles-secondary-text') +
                                        '</div>'
                                    );
                                } else {
                                    return (
                    ...
                    colors: [
                        $this.data('circles-bg-color') || '#377dff',
                        $this.data('circles-fg-color') || '#e7eaf3',
                    ],
                    duration: $this.data('circles-duration') || 1000,
                    wrpClass:
                        $this.data('circles-wrp-class') ||
                        self.config['wrpClass'],
                    textClass:
                        $this.data('circles-text-class') ||
                        self.config['textClass'],
                ...
                if ($this.data('circles-fg-stroke-linecap')) {
                    $this
                        .find(
                            '[class="' + self.config['valueStrokeClass'] + '"]'
                        )
                        .attr(
                            'stroke-linecap',
                            $this.data('circles-fg-stroke-linecap')
                        );
                }
                ...
                if ($this.data('circles-fg-stroke-miterlimit')) {
                    $this
                        ...
                        .attr(
                            'stroke-miterlimit',
                            $this.data('circles-fg-stroke-miterlimit')
                        );
                }
        ...
        _initAppear: function () {
            ...
            appear({
                ...
                appear: function (element) {
                    ...
                    element
                        .data('circle')
                        .update(element.data('reminded-value'));
                },
            });
        },
        ...
        get: function (index) {
            if (index && $.isNumeric(index))
                return this.pageCollection.eq(index);
         ...
         */
        getById: function (id) {
            if (id && this.pageCollection.filter('#' + id).length)
                return this.pageCollection.filter('#' + id);
        ...
        getCircleAPI: function (index) {
            if (
                index &&
                $.isNumeric(index) &&
                this.pageCollection.eq(index).length
            )
                return this.pageCollection.eq(index).data('circle');
        ...
        getCircleAPIById: function (id) {
            if (id && this.pageCollection.filter('#' + id).length)
                return this.pageCollection.filter('#' + id).data('circle');
