...
(function($) {
    ...
    $.HSCore.components.HSProgressBar = {
                ...
                init: function() {
                    $(selector).each(function(i, el) {
                        ...
                        if (config['direction'] === 'horizontal') {
                            $this.data(
                                'ProgressBar',
                                new self.HorizontalProgressBar($this, config)
                            );
                        } else {
                            $this.data(
                                'ProgressBar',
                                new self.VerticalProgressBar($this, config)
                            );
                        }
        ...
        HorizontalProgressBar: function(element, config) {
            ...
            this.config = config;
            this.moveElement = config['moveElementSelector']
                ? element.parent().find(config['moveElementSelector'])
                : $();
        ...
        extendHorizontalProgressBar: function() {
            ...
            this.HorizontalProgressBar.prototype.update = function(value) {
                ...
                if (this.config.useProgressElement) {
                    ...
                    if (this.moveElement.length)
                        this.config.beforeMoveElementUpdate.call(
                            this.moveElement
                        );

                    if (
                        self.config.moveElementSelector &&
                        self.config['moveElementTo'] == 'end'
                    ) {
                        var mCounter = 0,
                        ...
                        var mCounterId = setInterval(function() {
                            ...
                            if (self.moveElement.length)
                                self.config.onMoveElementChange.call(
                                    self.moveElement,
                                    (mCounter += mIterationValue)
                                );
                            ...
                            if (mCounter > 100) {
                                ...
                                if (self.moveElement.length)
                                    self.config.afterMoveElementUpdate.call(
                                        self.moveElement
                                    );
                            }
                        }, fps);
                    ...
                    this.element.data(
                        ...
                        setInterval(function() {
                            ...
                            if (
                                self.config.moveElementSelector &&
                                self.config['moveElementTo'] ==
                                    'currentPosition'
                            )
                                self.moveSubElement(currentValue);
        ...
        getAPI: function(index) {
            if (this._pageCollection.eq(index).length)
                return this._pageCollection.eq(index).data('ProgressBar');
